...
from database.models import Base, Users, YoutubePlaylist
...
class Database(commands.Cog):
    ...
    async def listsong(self, ctx):
        ...
        '''
        try:
            events = session.query(YoutubePlaylist).order_by(YoutubePlaylist.date).all()
            headers = ['Song', 'Requester', 'Date and Time']
            rows = [[e.song, e.user, e.date] for e in events]
    ...
    #@commands.command(pass_context=True)
    async def addsong(self, ctx, username: str, songname: str, urls: str):
        '''Creates a song with a specific url
            example: !addsong songname songurl
        '''

        try:
            # existing = session.query(YoutubePlaylist).filter(YoutubePlaylist.user == username, YoutubePlaylist.link == songurl).first()
            # if not existing:
                date_time = dt.datetime.now().strftime("%d/%m/%Y %I:%M")
                add_date = datetime.strptime(date_time, '%d/%m/%Y %I:%M')
                ytplaylist = YoutubePlaylist(user=username, song=songname, url=urls, date=add_date)
                session.add(ytplaylist)
                session.commit()
                await ctx.send('{} added successfully on playlist on {}'.format(username, add_date))
            # else:
